...
if [ X"${TSP_OPERATION}" = X"TSP_TUNNEL_TEARDOWN" ]; then
   ...
   sed -i '/# gogoc/,/nameserver/ d' /ramdisk/etc/resolv.conf
   if [ ! -z ${TSP_CLIENT_DNS_ADDRESS_IPV6} ]; then
     echo -e "# gogoc\nnameserver `ipv6_reformat ${TSP_CLIENT_DNS_ADDRESS_IPV6} 128`" >> /etc/resolv.conf
   fi
fi
...
if [ X"${TSP_HOST_TYPE}" = X"router" ]; then
   ...
   if [ "${LAN_V6_CONFIG}" != "off" ]; then
     ...
     echo " MaxRtrAdvInterval    15;" >> "$rtadvdconfigfile" 
     echo " MinRtrAdvInterval    5;" >> "$rtadvdconfigfile"
     if [ "${LAN_V6_TYPE}" = "stateful" ]; then
        echo " AdvManagedFlag       on;" >> "$rtadvdconfigfile"
     else
        echo " AdvManagedFlag       off;" >> "$rtadvdconfigfile"
     fi
  
     if [ "${LAN_V6_TYPE}" != "stateless" ]; then
        echo " AdvOtherConfigFlag   on;" >> "$rtadvdconfigfile"
     else
        echo " AdvOtherConfigFlag   off;" >> "$rtadvdconfigfile"
     fi
     ...
     if [ "${LAN_V6_TYPE}" != "stateful" ]; then
       ...
       echo "  AdvPreferredLifetime 30;" >> "$rtadvdconfigfile"
       echo "  AdvValidLifetime 40;" >> "$rtadvdconfigfile"
       echo " };" >> "$rtadvdconfigfile"
     fi
     ...
     if [ "${LAN_V6_DNS}" = "on" ]; then
       if [ ! -z ${TSP_CLIENT_DNS_ADDRESS_IPV6} ]; then
         if [ "${LAN_V6_TYPE}" = "stateless" ]; then
           echo "    RDNSS ${TSP_CLIENT_DNS_ADDRESS_IPV6} {" >> "$rtadvdconfigfile"
           echo "        AdvRDNSSOpen on;" >> "$rtadvdconfigfile"
           echo "    };" >> "$rtadvdconfigfile"
         fi
       fi
     else
       if [ X"${WAN_IPV6PRIDNS}" != X ]; then
         if [ "${LAN_V6_TYPE}" = "stateless" ]; then
           echo "    RDNSS ${WAN_IPV6PRIDNS} ${WAN_IPV6SECDNS} {" >> "$rtadvdconfigfile"
           echo "        AdvRDNSSOpen on;" >> "$rtadvdconfigfile"
           echo "    };" >> "$rtadvdconfigfile"
         fi
       fi
     ...
     if [ "${LAN_V6_TYPE}" != "stateless" ]; then
        if [ "${LAN_V6_DNS}" = "on" ]; then
          if [ ! -z ${TSP_CLIENT_DNS_ADDRESS_IPV6} ]; then
            if [ "${LAN_V6_TYPE}" != "stateless" ]; then
                PRIMARY_V6DNS=${TSP_CLIENT_DNS_ADDRESS_IPV6}
            fi
          fi
        else
        ...
        LAN_DHCP_START_V6=${TSP_PREFIX}::${LAN_DHCPV6START}
        LAN_DHCP_END_V6=${TSP_PREFIX}::${LAN_DHCPV6END}
        DHCPPD_PTIME=30
        DHCPPD_VTIME=40
