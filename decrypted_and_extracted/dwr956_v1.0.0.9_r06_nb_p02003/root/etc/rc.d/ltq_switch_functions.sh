...
# IMPORTANT NOTE: Both LAN and WAN (in case of MII1 and PTM) must be vlan interface. ATM WAN can be plain nasX
# input arguements
			...
			if [ "A${nvlan_id}A" = "AA" -o "A${nport_id}A" = "AA" ]; then
				...
				if [ "${waniface::3}" = "nas" ]; then
					#for ATM WAN ipstream traffic still goes through CPU path.
			    		config_port_member $wanvlan $nport_id 0
				else
					config_port_pvid $nport_id $wanvlan 0
					config_port_member $wanvlan $nport_id 0
				    	reset_port_member $wanvlan $cpu_port
					switch_cli IFX_FLOW_PCE_RULE_WRITE pattern.nIndex=`expr $pce_rule_start \+ $k` pattern.bEnable=1 pattern.bPortIdEnable=1 pattern.nPortId=$nport_id pattern.bMAC_DstEnable=1 pattern.nMAC_Dst=FF:FF:FF:FF:FF:FF action.eVLAN_Action=2 action.nVLAN_Id=$nvlan_id
	    				switch_cli IFX_FLOW_PCE_RULE_WRITE pattern.nIndex=`expr $pce_rule_start \+ $k \+ 1` pattern.bEnable=1 pattern.bPortIdEnable=1 pattern.nPortId=$nport_id pattern.bMAC_DstEnable=1 pattern.nMAC_Dst=$bridgemac action.eVLAN_Action=2 action.nVLAN_Id=$nvlan_id
	    				switch_cli IFX_FLOW_PCE_RULE_WRITE pattern.nIndex=`expr $pce_rule_start \+ $k \+ 2` pattern.bEnable=1 pattern.bPortIdEnable=1 pattern.nPortId=$nport_id pattern.bMAC_DstEnable=1 pattern.nMAC_Dst=01:00:5E:00:00:00 pattern.nMAC_DstMask=0x03F action.eVLAN_Action=2 action.nVLAN_Id=$nvlan_id
					k=`expr $k + 3`
				fi
		    	fi
		done
	...
	if [ "A${waniface}A" = "AA" -o "A${wanvlan}A" = "AA" ]; then
		...
		for lan_if in $( echo $2 | sed -n 1'p' | tr ',' '\n'); do
			...
			if [ "A${nvlan_id}A" = "AA" -o "A${nport_id}A" = "AA" ]; then
				...
				config_port_pvid $nport_id $nvlan_id 1
				reset_port_member $wanvlan $nport_id 
			    	config_port_member $wanvlan $cpu_port 1
