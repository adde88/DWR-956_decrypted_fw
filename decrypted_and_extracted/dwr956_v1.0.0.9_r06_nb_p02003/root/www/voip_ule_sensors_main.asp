...
if(document.sensorlist.psetnum.value =='' || isBlank(document.sensorlist.psetnum.value)){
        alert("Read Interval should not be  blank");
        return false;
    }

    if(isNaN(document.sensorlist.psetnum.value) || document.sensorlist.psetnum.value <=0 ){
        alert("Invalid Interval "+document.sensorlist.psetnum.value);
      return false;
    }
...
function submitSensorSettings(){
	...
	if( document.sensor.cpeid.value == 0){
